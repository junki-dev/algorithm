...
const solution = (k, priorityQueue, bag) => {
  ...
  for (let i = 0; i < k; i++) {
    ...
    let value = priorityQueue.dequeue(); //가방을 순회하면서 보석 무게보다 크거가 같은 경우는 그 보석을 담았다고 가정하고 //담은 가방을 제외한 가방을 리턴함
...
rl.on('line', function (line) {
  ...
  let [n, k] = input
    .shift()
    .split(' ')
    .map((el) => Number(el)); //우선순위 큐 생성
  const priorityQueue = new PriorityQueue();
  for (let i = 0; i < n; i++) {
    let [x, y] = input[i].split(' ').map((el) => Number(el)); //우선순위 큐에 보석 무게, 가격 담아줌
