const minWindow = function (s, t) {
  ...
  for (let char of t) {
    // Populate the map with chars of t with # of occurrences
    targetMap[char] = targetMap[char] == null ? 1 : targetMap[char] + 1;
  }

  while (right < s.length) {
    // Continue incrementing the right side of the window until it hits the end of the string
    ...
    while (charCounter === 0) {
      if (right - left < minWidth) {
        // first update the min references if the current right and left pointer are smaller
  ...
  return minWidth === Number.MAX_SAFE_INTEGER
    ? ''
